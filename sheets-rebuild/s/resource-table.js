class ResourceTable {
  ...
  placeholderImg =
    "https://storage.googleapis.com/resource-site-testing/activity-images/wmsi-octopus.png";
  ...
  localSearch() {
    ...
    let experience =
      $("#experience").val() != "" ? $("#experience").val() : null;
    ...
    return this.allResults.filter(
      (resource) =>
        (experience
          ? resource.Experience.split(", ").includes(experience)
          : true) &&
        (self_led ? resource.Tags.includes("self-led") : true) &&
        (subject ? resource.Subject.split(", ").includes(subject) : true) &&
        (search_string
          ? resource["Search Text"].includes(search_string)
          : true) &&
        (searchMaterials.length == Object.entries(this.materials).length ||
          searchMaterials.some((el) =>
            resource.Materials.split(", ").includes(el)
          ))
    );
  }
  ...
  getMaterialsLocal(searchMaterials) {
    for (const [key, value] of Object.entries(this.materials)) {
      if ($(key).is(":checked")) searchMaterials.push(value);
    }
  }
  ...
  */
  build(activities = this.searchResults) {
    var new_elements;
    const grid_item = "<span class='item'>*</span>";
    activities.forEach((resource, index) => {
      let activity_link = grid_item.replace(
        "*",
      ...
      const author_link =
        ...
        resource["Source"] +
        "</a>";
      const info_link = $("#info-icon-template").html().replace("@", index);

      if (resource["Tags"].includes("incomplete"))
        new_elements = this.adaptActivity(resource, index);
      // activity_link = this.adaptActivity(resource, index);
      else new_elements = activity_link;
      ...
      new_elements += grid_item.replace("*", info_link);

      $("#content").append(new_elements);

      if (resource["Tags"].includes("incomplete"))
        this.addAdaptModal(resource, index);
      else this.addModal(resource, index);

      if (ALLOW_COMMENTS) this.commentSection(resource, index);
    });
  }
  ...
  */
  commentSection(resource, index, key = "Comments") {
    var element =
      "<span class='item'>" + $("#comment-template").html() + "</span>";
    ...
    element = element
      .replace("*pos", 200 - index)
      .replace("*title", resource["Resource Name"])
      .replace("*link", resource["Resource Link"]);
    ...
    if (resource[key] != undefined) {
      ...
      comments.forEach((comment) => {
        markup += comment[0] + ": " + comment[1] + "<br>";
      });
      var preview =
        "<br>" + (markup.length > 60 ? markup.slice(0, 60) + "..." : markup);

      $("#comment-hover" + index + " b")
        .empty()
        .html("User comments preview:")
        .after(preview);
      $("#comment-text" + index + " h4")
        .empty()
        .html("User Comments:")
        .after(markup);
  ...
  /*
      Add Comment section to the bottom of a feature modal from the carousel
  ...
  */
  addFeatureComments(resource, index) {
    ...
    var element = $("#feature-comment-template")
      .html()
    ...
    if (resource.Comments != undefined) {
      ...
      comments.forEach((comment) => {
        $("#feature-comment-text" + index).append(
          comment[0] + ": " + comment[1] + "<br>"
        );
      });
    }

    $("#feature-new-comment" + index)
      .unbind("focus")
      .focus(() => {
        $(this).css("height", "90px");
        $(form_id).show();
  ...
  */
  postComment(resource, index, feature = false) {
    ...
    var id = feature
      ? "#feature-post-comment" + index
      : "#post-comment" + index;
    var comment_id = feature
      ? "#feature-new-comment" + index
      : "#new-comment" + index;
    var user_id = feature
      ? "#feature-comment-name" + index
      : "#comment-name" + index;
      ...
      .click(() => {
        ...
        if (comment != "") {
          ...
          $(markup_id)
            .empty()
            .append(
              "<b>Thanks for posting a comment! We will review your comment within the next 1-2 weeks and put it right here."
            );
  ...
  */
  renderFeatures() {
    ...
    if (this.featureList.length == 0) $("#feature-container").hide();
    else {
  ...
  */
  buildFeatures() {
    ...
    features.map((resource, i) => {
      ...
      if (ALLOW_COMMENTS) this.addFeatureComments(resource, i);
    });
  }
  ...
  */
  buildFeatureList() {
    ...
    this.featureList = this.searchResults.filter(
      (resource) => resource.Thumbnail && !resource.Tags.includes("incomplete")
    );
  ...
  */
  sortResults(callback) {
    $(".item-header i").click((e) => {
      ...
      if (field == "activity") this.sortText("Resource Name", ascending);
      if (field == "author") this.sortText("Source", ascending);
      if (field == "time") this.sortTime(ascending);
      if (field == "experience") this.sortExperience(ascending);
      if (field == "subject") this.sortText("Subject", ascending);
  ...
  */
  sortText(field, ascending) {
    ...
    else this.searchResults.sort((a, b) => b[field].localeCompare(a[field]));
  }
  ...
  */
  sortTime(ascending) {
    this.searchResults.sort((a, b) => {
      ...
      if (ascending) return a_time - b_time;
  ...
  */
  sortExperience(ascending) {
    ...
    this,
      this.searchResults.sort((a, b) => {
        var a_experience = a.Experience.includes(",")
          ? a.Experience.split(",")[0]
          : a.Experience;
        var b_experience = b.Experience.includes(",")
          ? b.Experience.split(",")[0]
          : b.Experience;
        if (ascending)
          return exp.indexOf(a_experience) - exp.indexOf(b_experience);
        else return exp.indexOf(b_experience) - exp.indexOf(a_experience);
      });
  }
  ...
  */
  displayMetaData(
    ...
    else if (num_results == 1) $("#results-meta").html("Displaying 1 Result.");
    else if (search_results.length < num_results) {
      // cycle through pages
      var start = page_size * page_num + 1;
      var end =
        page_size * (page_num + 1) < num_results
          ? page_size * (page_num + 1)
          : num_results;
      $("#results-meta").html(
        "Displaying " + start + "-" + end + " of " + num_results + " Results."
      );
    } else {
      $("#results-meta").html(
  ...
  ////////////////////////////// MODAL FUNCTIONS ////////////////////////////////////////
  /*
        Modify html template to create a modal with "Info" for an activity
        ...
        TODO: continue to evaluate what content fits best here
  */
  addModal(resource, index) {
    if (resource["Tags"].includes("incomplete"))
      return this.addAdaptModal(resource, index);

    var html_template = $("#info-modal-template").html();
    html_template = html_template.replaceAll("*id", "resource" + index);
  ...
  addAdaptModal(resource, index) {
    var html_template = $("#adapt-modal-template").html();
    html_template = html_template.replaceAll("*id", "resource" + index);
    html_template = html_template.replace("*class", "modal modal-grid");
  ...
  */
  ...
  /*
      Create a special modal for any activity that does not meet our lesson standards,
      and so requires adaptation by a teacher in order to be run as a classroom activity.
      All of these activities have an "incomplete" tag in the google sheet
  ...
  */
  adaptActivity(resource, index) {
    var markup = $("#adapt-modal-title").html();
